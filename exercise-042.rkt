...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname exercise-042) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
; the left border of the scene and the right-most edge car

; WorldState -> Image
; places the car into the BACKGROUND scene,
; according to the given world state
(check-expect
 (render 50) (place-image CAR (- 50 CENTER-TO-EDGE) Y-CAR BACKGROUND))
(check-expect
 (render 100) (place-image CAR (- 100 CENTER-TO-EDGE) Y-CAR BACKGROUND))
(check-expect
 (render 150) (place-image CAR (- 150 CENTER-TO-EDGE) Y-CAR BACKGROUND))
(check-expect
 (render 200) (place-image CAR (- 200 CENTER-TO-EDGE) Y-CAR BACKGROUND))
(define (render cw)
  (place-image CAR (- cw CENTER-TO-EDGE) Y-CAR BACKGROUND))
...
(define (end? cw)
  (if (> (- cw (image-width CAR)) BACKGROUND-WIDTH) #true #false))
